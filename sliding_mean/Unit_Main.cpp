...
typedef struct tag_SliderMean
{
    ...
    float   *SliderBuffer;
    double   *SliderBuffer_invertSumm;
...
void Init_SliderMean( TSliderMean *sm, int BuffersDepth )
{
    ...
    sm->SliderBuffer_invertSumm = (double*) malloc( sizeof(double) * BuffersDepth );
    sm->SliderBufferSumm        = (float)0.0;

    for ( i = 1; i <= BuffersDepth; i++) sm->SliderBuffer_invertSumm[i-1]  = 1.0 / (double)i;
    //for ( i = 1; i <= BuffersDepth; i++) sm->SliderBuffer_invertSumm[i-1]  = 1000.0 / (double)i;
//    for ( i = 1; i <= BuffersDepth; i++) sm->SliderBuffer_invertSumm[i-1] *= 0.001;
}
