...
//            }
  ...
  //***********************************************************************************
    ...
    TemplateType  *ValuesPtr(int index)
    {
...
public:
  ...
  // ******            = 2 Полная шапка
  // ******            = 3 Полная шапка (FileVersion, file_Param.FileVersion, SamplingTime, CanalCount, CanalNames)
          ...
          try {
               ...
               // ---- Сохранение (Валяем данные) -----------
                    ...
                    for ( int k = 0; k < Items[0]->ValuesCount; k++)
                    {
                        ...
                        for (int i = 0; i < ItemsCount; i++)
                        {
                            float val = Items[i]->Values[k];
                            if ( isFirstTab == true )
                            {
                                isFirstTab = false;
                                fprintf( h_file, "\n%1.9f", val );
                            } else
                                fprintf( h_file, "\t%1.9f", val );
                        }
                    }
          } catch ( ... ) {}
        } __finally  {
