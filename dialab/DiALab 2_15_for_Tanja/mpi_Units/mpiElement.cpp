...
#include "mpiElement_TVisualElementParam.h"
#include "mpiElement_TElementPoint.h"
#include "mpiElement_TPoints.h"
//===========================================================================
...
void TElement::RotateElemenet()
{
   ...
   // --- Тут надо инвертировать (поменять) размеры элемента ----------
      ...
      VisualElementParam->osdRect = Rect(-1, -1, -1, -1);
...
/*void TElement::DrawElementFace()
{
  // ---------- Родителський метод очмщающий под элементом подложку ----

    VisualElementParam->Bitmap->Canvas->Brush->Color = FonColor;
    VisualElementParam->Bitmap->Canvas->Pen  ->Color = FonColor;
    VisualElementParam->Bitmap->Canvas->FillRect(Rect(0,0, VisualElementParam->Width, VisualElementParam->Height));
}*/
...
bool TElement::IsElementInSelectedRegion(int aParentId, int ax1, int ay1, int ax2, int ay2)
{
    ...
    TRect  sr = VisualElementParam->sRect;
...
//== Проверка можно ли считать елемент (задел для разнотактовой системы) ====
//===========================================================================
bool TElement::isEnabledCalc(bool *isEnabledOneInput)
{
    // ---------- Опредиляем можно ли считать -------
       *isEnabledOneInput = false;
       bool afEnableCalc = true;
       for (int i = 0; i < iPoints->Items_Count; i++)
          if (iPoints->Items[i]->EnableCalc != NULL)
             if (!*iPoints->Items[i]->EnableCalc) afEnableCalc = false; else *isEnabledOneInput = true;
...
//===========================================================================
//===========================================================================
void TElement::LoadFromStream(TStream *aStream)  { TBaseClass::LoadFromStream(aStream);
  ...
  // ------------ Переворот Размеров, если елемент развернут -----
...
void TElement::SaveToStream(TStream *aStream) { TBaseClass::SaveToStream(aStream);
   int TempInt;
