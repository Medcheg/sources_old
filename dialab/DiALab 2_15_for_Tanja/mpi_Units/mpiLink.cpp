...
//===========================================================================
/*void TLinkPoint_init() { Beep(); }
#pragma startup TLinkPoint_init 110 // default 100*/
//===========================================================================
//===========================================================================
TLink    **LinkList;          // ---- Список Связей на схеме ---
int        LinkList_Count;    // ----- Количество Связей на схеме ----
//===========================================================================
...
void TLink::CorrectLinkValue()
{
  // ---------- Проверка на перпендикулярность, паралельность точек -------
      // --- Если во свей связи токо две точки и присутсвуют оба элемента то добавляем новые две точки --------
            ...
            TElement *FirstEL  = (TElement*)FirstElement;
            TElement *SecondEL = (TElement*)SecondElement;

      // ---- Ориентация второго элемента --------
            bool FirstHorizontal  = true;
            bool SecondHorizontal = true;
            if (FirstEL ->Orientation == oRight || FirstEL ->Orientation == oLeft  ) FirstHorizontal  = true;
            if (FirstEL ->Orientation == oTop   || FirstEL ->Orientation == oBottom) FirstHorizontal  = false;
            if (SecondEL->Orientation == oRight || SecondEL->Orientation == oLeft  ) SecondHorizontal = true;
            if (SecondEL->Orientation == oTop   || SecondEL->Orientation == oBottom) SecondHorizontal = false;
      ...
      // --- Если текущая и предыдущаи точки одинаковы по координатам , тогда нафиг -----
            if ( SecondHorizontal && fPoints[fPoints_Count - 1]->y == fPoints[fPoints_Count - 2]->y && fPoints[0]->y == fPoints[1]->y) return;
            if (!SecondHorizontal && fPoints[fPoints_Count - 1]->x == fPoints[fPoints_Count - 2]->x && fPoints[0]->x == fPoints[1]->x) return;

      // --- Иначе, (если больше трех точек) поправляем предыдущую точку ------

            if (  FirstHorizontal  ) fPoints[                1]->y = fPoints[                0]->y;
            if (  SecondHorizontal ) fPoints[fPoints_Count - 2]->y = fPoints[fPoints_Count - 1]->y;
            if ( !FirstHorizontal  ) fPoints[                1]->x = fPoints[                0]->x;
            if ( !SecondHorizontal ) fPoints[fPoints_Count - 2]->x = fPoints[fPoints_Count - 1]->x;
