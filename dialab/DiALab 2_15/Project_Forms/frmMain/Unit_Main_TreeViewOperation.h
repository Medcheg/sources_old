...
void TfrmMain::SetupTreeView()
{
 ...
 // --------
    ...
    TreeView1->Items->Clear();
 // ---- На данном этапе присвается только список элементов Nodu ---------
    for (int i = 0; i < TreeNodeList->Count; i++){
         TTreeNodeElement *tnElement = ((TTreeNodeElement*)TreeNodeList->Items[i]);

         // -------- Ищу родителя - если нету тода ставлю елемент в нулевой левел ---
         ...
         // -------- Если не нулевой уровень, то ищу по названию родителя и даю ему еще одного ребенка ----
           if (tnElement->ParentNodeText != "NULL") {
 ...
 // ---- НИже мулька для раскрыва первого крыла дерева ------
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
      ...
      // ----- Если нету список елементов в ветке дерева, то переходим на следу.щий елесент дерева ----
