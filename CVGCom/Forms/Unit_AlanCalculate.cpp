...
bool __fastcall TfrmAllanCalculateExtern::CheckEditedData()
{
 ...
 // ----------
    ...
    try { SamplingTime    = StrToFloat(eSamplingTime->Text); } catch (...) { erEdit = eSamplingTime;   }
...
void __fastcall TfrmAllanCalculateExtern::ChartClear()
{
   ...
   for ( int i = 0; i < Chart4->SeriesList->Count; i++) Chart3->Series[i]->Clear();
}
...
//===========================================================================
 void  __fastcall TfrmAllanCalculateExtern::CalculateRegressionTermoCorection( CCollection<float> *base,
...
//===========================================================================
float __fastcall TfrmAllanCalculateExtern::BICalcBySCO( int SummationCount, bool isCalcMean, CCollection<float> *InArray, TProgressBar *pb )
{
  // -------------------------------------
...
// ---------------------------------------------------------------------------
void  __fastcall TfrmAllanCalculateExtern::AlanCalc_Draw(double T0, CCollection<float> *aAlanData)
{
  ...
  // ------------
   ...
   // ------------
       ...
       Get_Regression_Coef( 2, MNKCoef, &XValues, 1, YValues, i2-i1, false);
   ...
   // ---- Отрисовка -----------
       ...
       for ( unsigned int i = 0; i < aAlanData->ValuesCount; i++)
       {
           Chart1->Series[0]->AddXY((i+1) * T0, aAlanData->Values[i]);
...
void __fastcall TfrmAllanCalculateExtern::CoefficientCorrectionOut()
{
   ...
   // ----- Стандартный вывод информации -------
        ...
        AnsiString as = "";
        as = "Zero_" + ComboBox_array[0]->Text + " = " + FloatToStrF(RegCoefZeros[0], ffExponent, 12, 2);
        Memo1->Lines->Add(as);
        for ( int i = 0; i < 6; i++ )
            if ( CheckBox_array[i]->Checked == true )
            {
                as = "Zero_" + ComboBox_array[i+1]->Text + " = " + FloatToStrF(RegCoefZeros[i+1], ffExponent, 12, 2);
                Memo1->Lines->Add(as);
            }
            ...
            if ( CheckBox_array[i]->Checked == true )
            {
                ...
                for ( int k = 0; k < PolinomLevel; k++ ) {
                    if ( CheckBox2->Checked == true ) {
                        Memo1->Lines->Add("\t c"+ IntToStr(k+1) +" = " + FloatToStrF(RegCoef[k + PolinomLevel*i], ffExponent, 12, 2));
                    } else {
                        Memo1->Lines->Add(FloatToStrF(RegCoef[k + PolinomLevel*i], ffExponent, 12, 2));
                    }
                }
            }
        }
   ...
   // ----- Вывод информации для матлаба -------
        ...
        for ( int i = 0; i < 6; i++ )
        {
            if ( CheckBox_array[i]->Checked == true )
            {
                ...
                as += " = [ "; 
		        for ( int k = 0; k < PolinomLevel; k++ )
        			as += FloatToStrF(RegCoef[k + PolinomLevel*i], ffExponent, 12, 2) + " ";
                as += "];";
    ...
    try
    {
          ...
          // --------- Интервалы исследования -----
           ...
           // ----- Маштаб входного сигнала ---------
            ...
            CCollection<float> *si_t = (CCollection<float> *)ComboBox_array[0]->Items->Objects[1];
            CCollection<float> temperature; temperature.Clear();

            //float ScaleCoef = 1.0;
            //if ( cbScaleFactor->Checked == true ) ScaleCoef = 1.0 / ScaleFactor;
            for ( int i = bIndex; i < eIndex; i++)
            {
                GyroOut.Add(si->Values[i]);
                temperature.Add(si_t->Values[i]);
           ...
           // ------------  Отрисовка входного сигнала ----------
            ...
            if ( cbScaleFactor->Checked == true ) ScaleCoef = 1.0 / ScaleFactor;
               ...
               {
                      DrawItem.Add        (GyroOut.Values[i] * HourCoef * ScaleCoef);
                      //CorrectionResult.Add(GyroOut.Values[i] * HourCoef * ScaleCoef);
               }
          ...
          // --- Опредиляю количество параметров по которому корректирую --------
           ...
           // ---- Результаты коррекции ----
            ...
            Draw_ChartSeries(Chart4, 0,  true, false, 3000, bIndex, 0, temperature.ValuesCount, NULL, &temperature);
          ...
          // ----------- Расчет ошибок по Алану ---------
              ...
              AlanCalc_Draw(SamplingTime, &AlanData);
    }
    __finally
    {
              bCalculate->Enabled = true;
...
void __fastcall TfrmAllanCalculateExtern::ChartAfterDraw(TObject *Sender)
{
...
//===========================================================================
void __fastcall TfrmAllanCalculateExtern::button_MNKClick(TObject *Sender)
{
	...
	Get_Regression_Coef( 2, MNKCoef, XValues, 1, YValues, 5, false);
  // -----------
...
//---------------------------------------------------------------------------
void __fastcall TfrmAllanCalculateExtern::button_ZerosClick(TObject *Sender)
{

    for ( int i = 0; i < 7; i ++)
    {
     	frm_AlanCalculateExtern_Zeros->ComboBox_array[i]->Items->Assign(ComboBox_array[i]->Items);
        frm_AlanCalculateExtern_Zeros->ComboBox_array[i]->ItemIndex = ComboBox_array[i]->ItemIndex;
        frm_AlanCalculateExtern_Zeros->ComboBox_array[i]->Enabled   = ComboBox_array[i]->Enabled;

        frm_AlanCalculateExtern_Zeros->EditBox_array[i]->Enabled = ComboBox_array[i]->Enabled;
    }

    frm_AlanCalculateExtern_Zeros->ShowModal();
}

//---------------------------------------------------------------------------
void __fastcall TfrmAllanCalculateExtern::cb_ZerosClick(TObject *Sender)
{
    button_Zeros->Enabled = cb_Zeros->Checked;
}

//---------------------------------------------------------------------------
void __fastcall TfrmAllanCalculateExtern::CheckBox2Click(TObject *Sender)
{
    CoefficientCorrectionOut();
}

//---------------------------------------------------------------------------
void __fastcall TfrmAllanCalculateExtern::CheckBox3Click(TObject *Sender)
{
    Chart4->Visible = CheckBox3->Checked;
}
