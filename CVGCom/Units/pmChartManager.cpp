...
CChartManager::CChartManager(Extctrls::TPanel *aParentPanel)
{
   ...
   // ----------
    ...
    ((TForm*)Panel_Parent->Parent)->ActiveControl = TabControl_SeriesTable;
            ...
            // -------- Panel_Chart - Init -------
            Panel_Main = new TPanel(Panel_Parent);
            Panel_Main->Parent         = TabControl_SeriesTable;
            Panel_Main->Align          = alClient;
            Panel_Main->BevelInner     = bvNone;
            Panel_Main->BevelOuter     = bvNone;
            //Panel_Main->BevelInner     = bvLowered;
            //Panel_Main->BevelOuter     = bvRaised;
            Panel_Main->BevelWidth     = 1;
            Panel_Main->Color          = clSilver;
            ...
            Panel_StringGrid->DoubleBuffered = true;
                    ...
                    StringGrid_Data->EditorMode       = true;
                    StringGrid_Data->Options << goEditing;
    ...
    Panel_SeriesNames->Parent     = Panel_Main; //Panel_Parent;
    ...
    Panel_Menu->Parent         = Panel_Main; //Panel_Parent;
...
void __fastcall CChartManager::TabControl_SeriesTable_onClick(TObject *Sender)
{
    ...
    Panel_Main->Visible       = ( TabControl_SeriesTable->TabIndex == 1 );
}
...
void __fastcall CChartManager::InitChartManagerInterface()
{
    ...
    // ----------------------------------
      ...
      // ----- Чистака таблици -------
        for ( int k = 1; k < StringGrid_Data->RowCount; k++) StringGrid_Data->Cells[0][StringGrid_Data->RowCount - k] = k;
        for ( int i = 1; i < StringGrid_Data->ColCount; i++)
        {
            for ( int k = 1; k < StringGrid_Data->RowCount; k++)
                StringGrid_Data->Cells[i][k] = "";
      ...
      // ----- Заполнение именами  -------
        if ( pStorage != NULL )
        {
            ...
            for (int i = 0; i < pStorage->ItemsCount; i++)
            {
                ...
                StringGrid_Data->Cells[CanalCounter + 1][0] = AnsiString(pStorage->Items[CanalCounter]->Name);

                CanalCounter++;
            }
        }

        //StringGrid_Data->Rows->EndUpdate();
        //StringGrid_Data->Cols->EndUpdate();
    ...
    // ------------------------------------
        ...
        if ( oldCountCheckedItems == 0 && CheckListBox_SeriesNames->Items->Count != 0 ) CheckListBox_SeriesNames->Checked[0] = true;
...
/*
        for ( int i = 1; i < StringGrid_Data->ColCount; i++)
        {
            sprintf(LabelText, "%1.9f\0", pStorage->Items[i-1]->LastValue);
            StringGrid_Data->Cells[i][1]  = LabelText;
        }
...
*/
    ...
    if ( isTableShow == false || isInterfaceInited == false ) Chart->Draw();
...
/*
        ...
        {
            int RowNumber = 0;
            for ( int k = pStorage->Items[i-1]->ValuesCount - 1; k >= 0 && RowNumber++ <= StringGrid_Data->RowCount; k-- )
            {
                sprintf(LabelText, "%1.9f\0", pStorage->Items[i-1]->Values[k]);
                StringGrid_Data->Cells[i][RowNumber] = LabelText;
                if ( i == 1) StringGrid_Data->Cells[0][RowNumber] = k;
            }
        }
        //StringGrid_Data->Rows->EndUpdate();
        //StringGrid_Data->Cols->EndUpdate();
