...
void __fastcall CChart::on_MouseDown_pb(System::TObject* Sender, TMouseButton Button, Classes::TShiftState Shift, int X, int Y)
{
  ...
  // -----------
    ...
    if ( isEditMode == false )
    {
        form_Result_1->SetupForms( pStorage, SelecetStorageParts, isSelecetStorageParts, &CountSelectedStorageParts );
        form_Result_1->Show();
        form_Result_1->FormStyle = fsMDIForm;
        Sleep(10);
        form_Result_1->TabControl1->TabIndex = CountSelectedStorageParts-1;
        form_Result_1->SetupTableValues( form_Result_1->TabControl1->TabIndex );
    }
    else
    {
