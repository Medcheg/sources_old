...
TFloat trouble(TFloat diri, TFloat highi)
{
  ...
  // ------
     ...
     TFloat e     = ArcTan2((highi - (ZvData.Ha + DeltaHa)), diri);
     TFloat hc    = sqrt(sqr(ZvData.Re + highi) + sqr(diri)) - ZvData.Re;
     TFloat drgii = 130.285839598937 * (sqrt((ZvData.Ha + DeltaHa)) + sqrt(hc)); // 4.12 * (sqrt(1000.0 * (ZvData.Ha + DeltaHa)) + sqrt(1000.0 * hc));
  // ------
     if (dc > drgii || e > dle1 || e < dle2) Result = 1.2345e-12;
       else {
              dnw = DN_SINb(ZvData.EpsLuch, e, ZvData.ShirLuch);
              dnw = 20.0 * log(dnw) / log(10.0);

              foid(dc, highi, ZvData.EpsLuch, ZvData.ShirLuch, ppol, eps, vij, DN_SINb);

              dne  = 40.0 * log(vij) / log(10.0);
              q1_2 = Qqr(q_op, dc, rh2);
             // ---

              q_2  = q1_2 + 2.0*dnw + dne;
              if (frmMain->cbCalcBaseDistance->Checked) q_2 = q_2 - ZvData.La - ZvData.Lj - 2 * ZvData.DeltaGa;
             // ---
...
//=======================       convex       ================================
//===========================================================================
TFloat convex(TFloat xx, TFloat yy)
{
  return trouble(xx,yy);
}
/*
procedure angle(z:real);      { угол в градусах }
  ...
  begin
   ...
   j:=2;
    repeat
      yy:=(ZvData.Ha + DeltaHa)+xx*ff; i:=i+j;  xx:=xx+j*x/x0;
...
void CalcMashtabCoef()
{
       ...
       CountSkipPoint = (ZvData.Esktrapoling)? 2 : 1;
...
void Draw_Net_and_Labels()
{
   ...
   // ----------------------------------
       ...
       MainCanvas->TextOut(aWindow.Right - 200, aWindow.Top + 10, "Ro = " + FloatToStrF(WorkBasicDistance, ffFixed, 20,2) + " km");
...
//      MainCanvas->Brush->Color = BgColor;
   ...
   // --------------------------------------------------------------------------------------------
/*     TFloat     WidthX = ZvData.MaxGorizontDistance;
     TFloat     WidthY = ZvData.MaxVerticalDistance;
     ...
     for (int i = 0; i < AnglesTickCount; i++) {
       ...
       // ------ Отрисовка угломестных линий -------
         if (ZvData.AnglesLine && i != 0 && i != AnglesTickCount - 1) {
             MainCanvas->Pen->Color = BackupColor;
             MainCanvas->Pen->Style = psDot;
             MainCanvas->Brush->Style = bsClear;
...
{
     // -------------------
       frmTable->Close(); Application->ProcessMessages();
     // -------------------
       Init();
     // ---------------------
       dle1         = ZvData.EpsLuch + 1.1*ZvData.ShirLuch;
       dle2         = ZvData.EpsLuch - 1.1*ZvData.ShirLuch;
     // ---------------------
       TFloat plt          = Power(10, -ZvData.Pfa);
       q_op_0              = Qfd(plt, 0.5); //{0.5} //
       q_op                = Qfd(plt, ZvData.Pd); //{0.5} //
       rh2                 = ZvData.BasicDistance * exp(q_op_0 / 40.0)/ exp(q_op / 40.0);
       //por          = thld(plt,sg0); //  // sg0          = 1.0;
     // ---------------------

     // --
       for (int i = 1; i < 50 + 1; i++) sn[i] = Data_S_N[i]; ///sn[i] = StrToFloat(FmMain.fbc.TextRC[i+2,1]);
     // --
       switch (ZvData.TypeSignalFluctuation) {
          case 1 : for (int i = 1; i < 50 + 1; i++) pp[i] = Kriv_obn[i][(ZvData.Pfa-2)*3+1]; break; // pp[i] = StrToFloat(FmMain.fbc.TextRC[i+2,(ZvData.Pfa-2)*3+2]);
          case 2 : for (int i = 1; i < 50 + 1; i++) pp[i] = Kriv_obn[i][(ZvData.Pfa-2)*3+2]; break; // pp[i] = StrToFloat(FmMain.fbc.TextRC[i+2,(ZvData.Pfa-2)*3+3]);
          case 3 : for (int i = 1; i < 50 + 1; i++) pp[i] = Kriv_obn[i][(ZvData.Pfa-2)*3+2]; break; // pp[i] = StrToFloat(FmMain.fbc.TextRC[i+2,(ZvData.Pfa-2)*3+3]);
          case 4 : for (int i = 1; i < 50 + 1; i++) pp[i] = Kriv_obn[i][(ZvData.Pfa-2)*3+3]; break; // pp[i] = StrToFloat(FmMain.fbc.TextRC[i+2,(ZvData.Pfa-2)*3+4]);
          case 5 : for (int i = 1; i < 50 + 1; i++) pp[i] = Kriv_obn[i][(ZvData.Pfa-2)*3+3]; break; // pp[i] = StrToFloat(FmMain.fbc.TextRC[i+2,(ZvData.Pfa-2)*3+4]);
       }
