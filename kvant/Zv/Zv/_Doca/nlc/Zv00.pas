...
var scstep,numscstepx,numscstepy,numstepy,i,k,aaaaaaaaa,ii,
    numsnig,ny1:word;
    x,y,y1,y2,stepx,stepy,ff,xx,yy,plt,sg0,por,
    { scstep-шаг по х и у }
    ko,dh,RADIUS,q_op,q1_1,rh1,rh2,wavelength,smoothness,antennaheigth:real;
...
begin
     ...
     if (dc>drgii) OR (e>dle1) OR (e<dle2)  then
       trouble:=1.2345e-12
             ...
             q_2:=q1_2+2*dnw+dne;
                if q_2>=q_op then Result:=0.5 else Result:=0;
          if FmMain.cbver.Checked then
           begin
            Result:=PPO(q_2,sn,pp);
  ...
  begin
   ...
   j:=2;
    repeat
      ...
      begin
       FmMain.ImRes.Canvas.TextRect(Rect(shift+i+45,0,shift+i+45+55,0+11),shift+i+45,0,st3+'°');
       //FmMain.ImRes.Canvas.TextOut(shift+i+45,0,st3+'°');
      end
     else
      ///outtextxy(640,round(yy*y0*y1/y),st3+'°');
      FmMain.ImRes.Canvas.TextRect(Rect(40,round(yy*y0*y1/y),40+55,round(yy*y0*y1/y)+10),40,round(yy*y0*y1/y),st3+'°');
      //FmMain.ImRes.Canvas.TextOut(40,round(yy*y0*y1/y),st3+'°');
   end;
procedure net;
   var iii:longint;
   RADIUS:real;
...
{}     iii:=0;
     repeat         {}
        ...
        ///putpixel(shift+round(cos(ff)*j*f*x0/x),y0-round(xx),15);
...
тогда надо его уменьшить. Для высоты 40 км нормальное число 500}
    ...
    begin
        ...
        FmMain.ImRes.Canvas.TextOut(shift+round(i*2*f/x*x0),ny1+9,st4)
    end;
    ///outtextxy(shift+x0-10,ny1-8,'R,km');
      FmMain.ImRes.Canvas.TextOut(shift+x0-20,ny1+8,'R,km');
    ...
    ///settextjustify(0,0);
       ff:=0;
       for aaaaaaaaa:=0 to {15}30 do begin
          i:=0; xx:=0;
                    repeat
             yy:=sqrt(sqr(RADIUS+ff)-xx*xx)-RADIUS-ff;
             j:=ny1-aaaaaaaaa*round(f*y0/y)-round(yy*y0/y);
             if aaaaaaaaa<>0 then FmMain.ImRes.Canvas.Pixels[i+shift,j]:=clWhite
             ...
             i:=i+4;xx:=xx+4*x/x0;
          until (j>y0) or (xx>x);
          str(aaaaaaaaa*f*1000:4:0,st4);
          if y0+8-aaaaaaaaa*round(f*y0/y)<20 then
          begin
           FmMain.ImRes.Canvas.TextOut(40,8,'H,m');
           //outtextxy(40,8,'H,m');
            end
          else
          //outtextxy(0,ny1+8-aaaaaaaaa*round(f*y0/y),st4);
          FmMain.ImRes.Canvas.TextOut(0,ny1+8-aaaaaaaaa*round(f*y0/y)-10,st4);
          ff:=ff+f;
       end;
       ///outtextxy(40,8,'H,m');
       FmMain.ImRes.Canvas.TextOut(40,8,'H,m');
...
{       angle(elper*180/pi);}
 ...
 if FmMain.cbver.Checked then
  begin
   /////FmMain.ImRes.Canvas.TextRect(Rect(40,270,40+55,270+10),40,450,'Detection');
    ///FmMain.ImRes.Canvas.TextRect(Rect(40,450,40+55,450+10),40,450,'Detection');
    ///FmMain.ImRes.Canvas.TextRect(Rect(40,460,40+55,460+10),40,460,'Probability');
   FmMain.ImRes.Canvas.TextOut(40,440,'Detection'); FmMain.ImRes.Canvas.TextOut(40,460,'Probability');
    ...
    FOR I:=1 TO 8 DO begin
        ...
        FmMain.ImRes.Canvas.Font.Color:=clWhite;
         FmMain.ImRes.Canvas.TextOut(40*i+110,460,st); //outtextxy(40*i+110,470,st);
    end;
   ...
   //FmMain.ImRes.Canvas.TextOut(88,38,'Surface');  /// outtextxy(shift+8,ny1+38,'Surface');
  end;
...
var     i,j,pfai:integer;
 begin
   rh1:=StrToFloat(FmMain.edDal.Text); //20.1;
     initzonvidvar;
     elper:=StrToFloat(FmMain.edLuch.Text) {0.8};elpri:=elper;
      elper:=elper*pi/180;
     sdnper:=StrToFloat(FmMain.edShir.Text) {3.0};sdnpri:=sdnper;
           y1:=FmMain.spHNlc.Value*1e-3; {30e-3};
           drg:=4.12*(sqrt(ha)+sqrt(y1*1000));
           ddd:=4.12*sqrt(y1*1000);
                      ddd:=4.12*sqrt(Ha)+1;
     ccc:=(14.3*lam/ha*0.5)*pi/180;
     x:=0.8*drg; { x:=rh1*1.65;}
     y2:=sqr(x{ddd})/(2*q0[11]);
     y:=y1+y2;
     ny1:=round(y0*y1/y);
...
{* функция на прямоугольнике 0-х на 0-у ; scstep- шаг в точках экрана*}
    ...
    mfl:=FmMain.rgMFS.ItemIndex+1 {1};
            for i := 1 to 50 do ///sn[i]:=StrToFloat(FmMain.fbc.TextRC[i+2,1]);
                                sn[i]:=data_s_n[i];
            pfai:=StrToInt(FmMain.edPfa.Text);
            case  mfl of
            1:   for i := 1 to 50 do pp[i]:=Kriv_obn[i,(pfai-2)*3+1];  ///pp[i]:=StrToFloat(FmMain.fbc.TextRC[i+2,(pfai-2)*3+2]);
            2,3: for i := 1 to 50 do pp[i]:=Kriv_obn[i,(pfai-2)*3+2]; ///pp[i]:=StrToFloat(FmMain.fbc.TextRC[i+2,(pfai-2)*3+3]);
            4,5: for i := 1 to 50 do pp[i]:=Kriv_obn[i,(pfai-2)*3+3]; ///pp[i]:=StrToFloat(FmMain.fbc.TextRC[i+2,(pfai-2)*3+4]);

            end;
    nak:=1;           sg0:=1.0;       nm:=mfl;
        por:=thld(plt,sg0);
        q_op:=qfd(plt,StrToFloat(FmMain.edPd.Text){0.5});
       ...
       FmMain.PB.Position:=Round(i*100/numscstepx);
          ...
          for j:=0 to numscstepy-1 do
              begin
                   ...
                   arrcurr[aaaaaaaaa+k]:=(arrcurr[aaaaaaaaa]*(scstep-k)+arrcurr[aaaaaaaaa+scstep]*k)/scstep;
              end;
         {**interpolated current values by y**}  {**now step by x**}
          for k:=0 to scstep-1 do
          for aaaaaaaaa:=0 to ny1+numsnig do
              begin
                   ff:=(arrsav[aaaaaaaaa]*(scstep-k)+arrcurr[aaaaaaaaa]*k)/scstep;
                   //ff:=(arrsav[aaaaaaaaa]*(scstep-k)+arrcurr[aaaaaaaaa]*k);
                   colnum:=calccolnum(ff);
                   FmMain.ImRes.Canvas.Pixels[shift+i * scstep+k-scstep,aaaaaaaaa]:=colnum;

              end;{**by aaaaaaaaa**}
          for aaaaaaaaa:=0 to ny1+numsnig do arrsav[aaaaaaaaa]:=arrcurr[aaaaaaaaa];
     end;{**by i**}
