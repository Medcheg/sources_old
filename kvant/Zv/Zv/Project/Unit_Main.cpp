...
{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                SaveDTSParam();
     ...
     CGauge1->Height = StatusBar1->Height - 2;
     CGauge1->Top    = 3;
     CGauge1->Left   = 2;
     CGauge1->Width  = Panel1->Width - 2 * CGauge1->Left;
   ...
   // -----------
      TIniFile *IniFile = new TIniFile(ExtractFilePath(Application->ExeName)+"zv.ini");
        ...
        AnglesLine   =                IniFile->ReadBool   ("Indicator", "AnglesLine" , false);
        Esktrapoling =                IniFile->ReadBool   ("Indicator", "AnglesLine" , false);
        AnglesLine   =                IniFile->ReadBool   ("Indicator", "Extrapolation" , false);
...
void __fastcall TfrmMain::FormDestroy(TObject *Sender)
{
   ...
   TIniFile *IniFile = new TIniFile(ExtractFilePath(Application->ExeName)+"zv.ini");
    ...
    IniFile->WriteBool   ("Indicator", "AnglesLine" , Esktrapoling);
    IniFile->WriteBool   ("Indicator", "Extrapolation" , AnglesLine);
...
void __fastcall TfrmMain::aAllAction(TObject *Sender)
{
     switch (((TAction*)Sender)->Tag) {
        case 0    : if (CheckInputParam()) frmTable->Close(); DeltaHa   = 0; Stoped = false;  RunModel(); Stoped = false; Repaint();break;
        case 1    : if (CheckInputParam()) Stoped = false; RunDynamicModel(); Stoped = false; Repaint();break;
        ...
        case 1000 : if (!DynamicModelRun) Esktrapoling     = cbEsktrapoling    ->Checked; break;
        case 1003 : if (!DynamicModelRun) AnglesLine       = cbAnglesLine      ->Checked; Repaint();break;
...
void TfrmMain::SetProjectParam()
{
   // -----  Параметры РЛС --------
     ...
     eShirLuchV        ->Text    = FloatToStr(ZvData.ShirLuchV * _180_del_pi);
     eShirLuchG        ->Text    = FloatToStr(ZvData.ShirLuchG * _180_del_pi);
     eLambda           ->Text    = FloatToStr(ZvData.Lambda *  100.0 * 1000.0);
     eOmegaA           ->Text    = FloatToStr(ZvData.OmegaA * _180_del_pi);
     eFreqFollowImpuls ->Text    = FloatToStr(ZvData.FreqFollowImpuls);
     eImpulsCount      ->Text    = IntToStr(ZvData.ImpulsCount);
     eBasicDistance    ->Text    = FloatToStr(ZvData.BasicDistance);
     eNju              ->Text    = FloatToStr(ZvData.Nju_Edit);
  ...
  //----------------------------
     cbEsktrapoling    ->Checked = Esktrapoling;
     cbAnglesLine      ->Checked = AnglesLine;
...
bool TfrmMain::CheckInputParam()
{
   // -----  Параметры РЛС --------
     ...
     try { ZvData.ShirLuchV        = StrToFloat(eShirLuchV       ->Text) * pi_del_180;     } catch ( ... ) {ShowWrongEdit(eShirLuchV);        return false; }
     try { ZvData.ShirLuchG        = StrToFloat(eShirLuchG       ->Text) * pi_del_180;     } catch ( ... ) {ShowWrongEdit(eShirLuchG);        return false; }
     ...
     try { ZvData.FreqFollowImpuls = StrToFloat(eFreqFollowImpuls->Text);                  } catch ( ... ) {ShowWrongEdit(eFreqFollowImpuls); return false; }
     try { ZvData.ImpulsCount      = StrToFloat(eImpulsCount     ->Text);                  } catch ( ... ) {ShowWrongEdit(eImpulsCount);      return false; }
     try { ZvData.Nju_Edit         = StrToFloat(eNju             ->Text);                  } catch ( ... ) {ShowWrongEdit(eNju);              return false; }
  ...
  // ------ Расчет количечства импульсов если задан их автоматический расчет  ------ 
     if (!CheckBox1->Checked) {
         ZvData.ImpulsCount =  ZvData.FreqFollowImpuls / (ZvData.OmegaA / ZvData.ShirLuchG);
         eImpulsCount->Text =  ZvData.ImpulsCount;
     }
  ...
  // ------ Выбор параметра опорной дальности ------
     if (!cbCalcBaseDistance->Checked) WorkBasicDistance = ZvData.BasicDistance;
        ...
        if (!CheckBox2->Checked) {
             float  plt  = Power(10, -ZvData.Pfa);
             float  q_op = Qfd(plt, ZvData.Pd); //{0.5} //
                    nju  = q_op;
        } else nju = ZvData.Nju_Edit;

        ZvData.BasicDistance  = 10*log10(ZvData.Pt) + 10*log10(ZvData.DSink) + 2*10*log10(ZvData.Lambda*1000.0*100.0) + 10*log10(ZvData.TargetEPR) + ZvData.Gt  + ZvData.Gr;
        ZvData.BasicDistance  = ZvData.BasicDistance - ZvData.NFSink - 10*log10(ZvData.BSink) - ZvData.Lt - ZvData.Lr - ZvData.Ls - ZvData.ASink;
        WorkBasicDistance     = ZvData.BasicDistance - nju - ZvData.La - ZvData.Lj - 2 * ZvData.DeltaGa;

        WorkBasicDistance    = Power(10, WorkBasicDistance    / 10.0 / 4.0);
        ZvData.BasicDistance = Power(10, ZvData.BasicDistance / 10.0 / 4.0);
     }
  ...
  // ----- еСЛИ АВТОМАТИЧЕСКИЙ РАСЧЕТ высоты отображения ---------
     if (cbAutoMaxVerticalDistance->Checked) {
        ZvData.MaxVerticalDistance = WorkBasicDistance * sin(ZvData.EpsLuch + ZvData.ShirLuchV / 2.0);
...
{
     if ( cbCalcBaseDistance->Checked) {
              OldStatusCheckBox2 = CheckBox2->Checked; CheckBox2->Checked = false;
              //OldStatusCheckBox1 = CheckBox1->Checked; CheckBox1->Checked = false;
     }
     if (!cbCalcBaseDistance->Checked)   {
              CheckBox2->Checked = OldStatusCheckBox2;
              //CheckBox1->Checked = OldStatusCheckBox1;
     }

     CheckBox2->Enabled = cbCalcBaseDistance->Checked;
     eNju     ->Enabled = CheckBox2->Checked;
     Label6   ->Enabled = cbCalcBaseDistance->Checked;
//     CheckBox1->Enabled = cbCalcBaseDistance->Checked;
  //   eImpulsCount->Enabled = cbCalcBaseDistance->Checked;

/*     if ( cbCalcBaseDistance->Checked) { OldStatusCheckBox2 = CheckBox2->Checked; CheckBox2->Checked = false; }
     if (!cbCalcBaseDistance->Checked)   CheckBox2->Checked = OldStatusCheckBox2;
...
void TfrmMain::Set_DefaultParam()
{
     // ------------- Параметры РЛС --------
        ...
        ZvData.ShirLuchV               = 3.0    * pi_del_180;    // ---- Ширина Луча ------
        ZvData.ShirLuchG               = 3.0    * pi_del_180;    // ---- Ширина Луча ------
        ZvData.OmegaA                  = 72.0   * pi_del_180;   // ---- Угловая скорость вращения анетены -----
        ZvData.FreqFollowImpuls        = 2000;
        ZvData.Nju_Edit                = 0;

     // ---- Тут нужно вставить параметр формы диаграммы направлености ------
        ZvData.BasicDistance           = 20.1;    // ----  Опорная дальность ------------
...
//        FontColor               = clWhite;
      ...
      // ----------------
        ...
        ZvData.OtherPd                 = false;      // FmMain.cbver.Checked
}
//---------------------------------------------------------------------------
...
//    tbSetting->Caption      = LanguageString[1][LanguageType];   //    Установки
  ...
  // -----
    ...
    Panel2->Caption               = " " + LanguageString[27][LanguageType];    //   "Перечень зон видимости:";
    ...
    Label12->Caption              = LanguageString[33][LanguageType];    //   "Максимальная дальность";
...
// --------------------------- frmTable -------------
  ...
  // ----------------
      ...
      //CheckBox1->Caption      = LanguageString[108][LanguageType];
      CheckBox1->Caption      = LanguageString[109][LanguageType];
      eImpulsCount->Left      = CheckBox1->Left + Canvas->TextWidth(CheckBox1->Caption) + 16 + 8;

      //      Label82->Caption        = LanguageString[109][LanguageType];
      Label57->Caption        = LanguageString[110][LanguageType];

      Label70->Caption        = LanguageString[111][LanguageType];
      Label80->Caption        = LanguageString[112][LanguageType];
      CheckBox2->Caption      = LanguageString[113][LanguageType];
      eNju->Left              = CheckBox2->Left + Canvas->TextWidth(CheckBox2->Caption) + 16 + 8;
      Label6->Left            = eNju->BoundsRect.Right + 4;
      Label6->Caption         = LanguageString[45][LanguageType];    //   "дБ";
...
{
//     Label82          ->Enabled = !CheckBox1->Checked;
     eImpulsCount     ->Enabled =  CheckBox1->Checked;
     Label80          ->Enabled =  !CheckBox1->Checked;
     eFreqFollowImpuls->Enabled =  !CheckBox1->Checked;
     Label81          ->Enabled =  !CheckBox1->Checked;
     Label70          ->Enabled =  !CheckBox1->Checked;
     eShirLuchG       ->Enabled =  !CheckBox1->Checked;
     Label78          ->Enabled =  !CheckBox1->Checked;
}
...
{
   if ( CheckBox2->Checked) { OldStatusCheckBox1 = CheckBox1->Checked; CheckBox1->Checked = true; }
   if (!CheckBox2->Checked)   CheckBox1->Checked = OldStatusCheckBox1;

   CheckBox1->Enabled = !CheckBox2->Checked;
   eNju     ->Enabled =  CheckBox2->Checked;
   Label38  ->Enabled = !CheckBox2->Checked;
   rbFljuk0 ->Enabled = !CheckBox2->Checked;
   rbFljuk1 ->Enabled = !CheckBox2->Checked;
   rbFljuk2 ->Enabled = !CheckBox2->Checked;
   rbFljuk3 ->Enabled = !CheckBox2->Checked;
   rbFljuk4 ->Enabled = !CheckBox2->Checked;
}
//---------------------------------------------------------------------------
