...
void __fastcall TfrmFiltrParam::FormShow(TObject *Sender)
{
    ...
    efK1   ->Set_Float_Value(Kalman->k1);
    efK2   ->Set_Float_Value(Kalman->k2);
    efN    ->Set_Int_Value  (DST1   ->N);
    efT0   ->Set_Float_Value(T0);
    efT    ->Set_Float_Value(EndCalcTime);
    efEndTimeCoefN->Set_Float_Value(EndTimeCoefN);
...
{
  bool Result = true;
    efK1   ->Get_Float_Value(&Kalman->k1, &Result);     if (!Result) return false;
    efK2   ->Get_Float_Value(&Kalman->k2, &Result);     if (!Result) return false;
    efN    ->Get_Int_Value  (&DST1   ->N, &Result);     if (!Result) return false;
    efT0   ->Get_Float_Value(&T0, &Result);             if (!Result) return false;
    efT    ->Get_Float_Value(&EndCalcTime, &Result);    if (!Result) return false;
    efEndTimeCoefN->Get_Float_Value(&EndTimeCoefN, &Result);    if (!Result) return false;
