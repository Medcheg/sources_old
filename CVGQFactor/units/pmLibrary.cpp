...
//===========================================================================
int __fastcall GetFileSize( FILE *h_file )
{
    int FileCurrentPosition = ftell(h_file);
    fseek(h_file, 0L, SEEK_END);
    int FileSize = ftell(h_file);
...
//===========================================================================
void __fastcall Draw_ChartSeries(TChart *aChart, int SeriesIndex,
                                 bool isGraphDecimation, bool isShowLeftAxisTitle,
                                 int DecimationPointCount, float LeftAxisValueOffset,
                                 int iBeginArrayIndex, int iEndArrayIndex,
                                 CCollection<float> *ax, CCollection<float> *ay)
{
  ...
  // ----------
	...
	if ( ax != NULL ) ax->CalculateGraphicsValues( isGraphDecimation, true, iBeginArrayIndex, iEndArrayIndex, DecimationPointCount );
    if ( ay != NULL ) ay->CalculateGraphicsValues( isGraphDecimation, true, iBeginArrayIndex, iEndArrayIndex, DecimationPointCount );
...
//===========================================================================
void __fastcall Draw_ChartSeries_simple(TChart *aChart, int SeriesIndex,
										bool isGraphDecimation, bool isShowLeftAxisTitle,
										int DecimationPointCount, float LeftAxisValueOffset,
										int iBeginArrayIndex, int iEndArrayIndex,
                                 		CCollection<float> *ax, CCollection<float> *ay)
{
  ...
  // ----------
	...
	if ( ax != NULL ) xVal = ax->Get_Array();
    if ( ay != NULL ) yVal = ay->Get_Array();
	//if ( ax != NULL ) ax->CalculateGraphicsValues( isGraphDecimation, true, iBeginArrayIndex, iEndArrayIndex, DecimationPointCount );
    //if ( ay != NULL ) ay->CalculateGraphicsValues( isGraphDecimation, true, iBeginArrayIndex, iEndArrayIndex, DecimationPointCount );
	...
	float ArrayStep = ( isGraphDecimation == false ) ? 1 : (iEndArrayIndex - iBeginArrayIndex) / (float)DecimationPointCount;
	if ( ArrayStep < 1 ) ArrayStep = 1;
	for ( float index = 0; index < (int)ay->ValuesCount; index = index + ArrayStep )
	{
        int i = index;
        if ( ax != NULL )
        {
        ...
        if ( ax == NULL )
			 aChart->Series[SeriesIndex]->AddXY(LeftAxisValueOffset + index  , yVal[i]);
        else aChart->Series[SeriesIndex]->AddXY(LeftAxisValueOffset + xVal[i], yVal[i]);
    }
