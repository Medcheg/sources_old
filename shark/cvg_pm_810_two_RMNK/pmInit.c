...
void Init_DebugMode ( TGyro *gyro, TDebug *debug )
{
    ...
    debug->debug_value     = &gyro->rmnk.Qy[1];
    ...
    for ( iCounter = 0; iCounter < DEBUG_ARRAY_DEPTH; iCounter++)
    {
        ...
        debug->param2[iCounter] = (float)0.0;
    }
...
void Init_UART_Param( TGyro *gyro )
{
    ...
    gyro->UART_Param[ 9] = &gyro->rmnk.Qy[1];// --- gyro->ExcitationPhase;
    gyro->UART_Param[10] = &gyro->rmnk.Qy[2];// --- gyro->CompensationPhase;
