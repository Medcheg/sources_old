...
/////////////
   ...
   // ----------------- Пегуляторы ------
    Init_PiReg( &gyro->pi_am_1, (float)X_ampl*X_ampl, Kp_x1, Ki_x1, 0.0, (float)DISCRET_TIME );  
   ...
   // -------------------------
    ...
    gyro->TimeToSwitch              = (float)15.0;
    gyro->CurrentTimeToSwitch1      = (float)0.0;
    gyro->CurrentTimeToSwitch2      = (float)0.0;
    gyro->is_XCanalExcitation_float = (float)1.0;
...
void Init_RMNK( TRMNK *prmnk, float DefaultPeriod, float BaseXAmpl, float DefaultFreq )
{
    float  period_T0, period_T, period_T2, period_D, period_a0,  period_b1;
    ...
    Init_ZForm( &prmnk->fi_Xc, DefaultPeriod );
    ...
    // ---- Коефициенты фильтра параметра B -----
    period_T0 = (float)1.0 / DefaultFreq;
    period_T  = (float)1.0 / (TWO_PI * (float)14.0);
    period_T2 = 2*period_T;
    period_D  = period_T2 + period_T0;
    period_a0 =  period_T0 / period_D;
    period_b1 = (period_T0 - period_T2) / period_D;

    prmnk->fi_Xc.a[0] = period_a0;
    prmnk->fi_Xc.a[1] = period_a0;
    prmnk->fi_Xc.b[0] = (float)1.0;
    prmnk->fi_Xc.b[1] = period_b1;

// Specify Order 1
// Fs = 100000
// Fc = 250
//a1 = 0.0077929362919515527 
//a2 = 0.0077929362919515527

//b1 = 1.000000000000000
//b2 = -0.98441412741609691 


// Specify Order 1
// Fs = 100000
// Fc = 100
//a1 = 0.0031317642291927056
//a2 = 0.0031317642291927056

//b1 = 1.000000000000000
//b2 = -0.9937364715416146  
}
...
void Init_UART_Param( TGyro *gyro )
{
    ...
    gyro->UART_Param[17] = &gyro->is_XCanalExcitation_float;//gyro->NULL_value;
