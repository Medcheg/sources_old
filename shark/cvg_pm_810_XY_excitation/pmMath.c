...
{
    float cos_freq, sc_argument, local_var, buble;
    Sint8 iCounter;

    if ( gyro->is_XCanalExcitation_float < (float)0.5 )
    {
        buble = gyro->in_Ux;
        gyro->in_Ux = gyro->in_Uy;
        gyro->in_Uy = buble;
    }
   ...
   // --- 2 ------ ДАННЫЕ В UART -------
    if ( gyro->isEnabledUARTData == 1 )  Setup_UART_data( gyro );
   ...
   // --- 6 ------- Демодуляция Кориолиса и Квадратуры ---
    ...
    Run_Demod( &gyro->fi_demod_q, - gyro->fi_input_y.BlockOut * cosf( gyro->rmnk.x_phi + gyro->PhiQPhase ) );//
  ...
  // ---   ------- тогда переходим на компенсирование узла
   ...
   // --- 11 ----------- Управляющие сигналы --------------------------
    ...
    gyro->CurrentTimeToSwitch1 = gyro->CurrentTimeToSwitch1 + DISCRET_TIME;
    if ( gyro->CurrentTimeToSwitch1 >= (float)10.0 ) 
    {
        gyro->CurrentTimeToSwitch1 = (float)0.0;
        gyro->CurrentTimeToSwitch2 = gyro->CurrentTimeToSwitch2 + (float)1.0;
        if ( gyro->CurrentTimeToSwitch2 >= (float)6.0 )
        {
            gyro->isEnabledUARTData = 1;
        }

        if ( gyro->CurrentTimeToSwitch2 >= (float)15.0 )
        {
            gyro->CurrentTimeToSwitch2       = (float)0.0;
            gyro->is_XCanalExcitation_float *= (float)-1.0;
            gyro->isEnabledUARTData = 0;
        }
    }
   ...
   // ---- 16 --------- Пошаговая система расчета коррекций ------------
    ...
    {
        sc_argument = corr_mk0_offset + corr_mk1_f1 * gyro->f1 + corr_mk1_f2 * gyro->f2 + corr_mk1_f3 * gyro->f3
