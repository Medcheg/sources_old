...
/////////////
    ...
    gyro->isDeltaPhi_corr          = CORR_DELTA_PHI;
...
/////////////
    ...
    gyro->Base_ANodeAmpl         = (float)X_ampl*(float)X_ampl;
    ...
    gyro->Base_DeltaPhi          = (float)PHASE_DELTA_PHI    * DEG_TO_RAD;
    ...
    gyro->DeltaPhi          = gyro->Base_DeltaPhi;
   ...
   // ----------------- Пегуляторы ------
    ...
    Init_PiReg_new( gyro->pi_k_exc , (float)          0.0, Kp_q1, Ki_q1, (float)DISCRET_TIME );
   ...
   // ----------------- Задержки ------
    ...
    Init_PhaseDetector( &gyro->Phase_Ecx_An, gyro->DefaultFrequency);
    Init_PhaseDetector( &gyro->Phase_X     , gyro->DefaultFrequency);
...
void Init_DebugMode ( TGyro *gyro, TDebug *debug )
{
    ...
    {
        debug->debug_value    = &gyro->DeltaPhi;
...
//***** debug    
    gyro->UART_Param[ 5] = &gyro->tEMPoMEGA;//gyro->GyroOutRaw;      
//***** debug    
...
//***** debug    
    gyro->UART_Param[14] = &gyro->NULL_value;                  //gyro->rmnk.XFrequency;
    gyro->UART_Param[15] = &gyro->Phase_X.Phase_deg;       //gyro->pi_quad_2[0];
    gyro->UART_Param[16] = &gyro->Phase_Ecx_An.Phase_deg; //DeltaPhi; //gyro->rmnk.XFrequency_m;
//***** debug    
