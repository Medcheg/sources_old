...
void SetupAlgo( TGyro *gyro )
{
    ...
    // ------ Запоминаю высокочастотную инфу -------------
    if ( gyro->DebugMode < 6 )
    {
...
/*
    gyro->SummOmega += gyro->pi_corr_2[0];
    if (  ++gyro->OmegaCounter > 1000 ){
        gyro->SummOmega *= 0.001;
        gyro->OmegaCounter = 0;
        
        if ( ABS(gyro->SummOmega) >= 9.5 ) {
            if ( gyro->SummOmega != 0 )
                kkk = ABS(9.5 / gyro->SummOmega);
        } else kkk = 1.0;
        ...
        gyro->NULL_value = kkk;
        gyro->pi_ampl_1[3] = gyro->Base_ANodeAmpl * gyro->NULL_value;
        //gyro->NULL_value = gyro->SummOmega;
    }
    
*/    
    //if ( gyro->rmnk.YAmpl >= 3.0 )  
