...
void Init_PeriodDetector    ( TPeriodDetector *pd, Sint16 val1, float DefaultPeriod );
...
void Init_Algo( TGyro *gyro, float *apUARTfloats  )
{
    ...
    gyro->DefaultPeriod_x          = T_base_x;
    gyro->DefaultPeriod_y          = T_base_y;
    ...
    // ----------------- Пегуляторы ------
    Init_PiReg( &gyro->pi_am_1 , X_ampl, Kp_x1, Ki_x1,  X_ampl );
    ...
    Init_PiReg( &gyro->pi_phase_x,  0.0,   0.1, 50.0, 0.0 );
    ...
    //Init_InputMean ( &gyro->XPhase     ,  10, 50, (float)ALGO_FREQUENCY / gyro->DefaultPeriod, 0.0 );

    Init_PeriodDetector ( &gyro->ANodePeriod, 100, gyro->DefaultPeriod_x );
    ...
    Init_Zn( &gyro->Zn_cos1   , (float)0.25 * T_base_x       );

    Init_Zn( &gyro->Zn_sin2_x , N_comp_x                     );
    Init_Zn( &gyro->Zn_sin2_y , N_comp_y                     );
    Init_Zn( &gyro->Zn_cos2_x , (float)0.25 * T_base_x + N_comp_x);
    Init_Zn( &gyro->Zn_cos2_y , (float)0.25 * T_base_x + N_comp_y);
    ...
    gyro->DefaultPeriod_invert_x = (float)1.0 / (float)gyro->DefaultPeriod_x;
    gyro->DefaultPeriod_invert_y = (float)1.0 / (float)gyro->DefaultPeriod_y;
    ...
    gyro->amin = (float) 123.0;
    gyro->amax = (float)-12.03;
    gyro->isWorkCanalX = 1;
...
void Init_BandPassFiltration( TGyro *gyro )
{
    ...
    filter_B           = (float)2.0 * cosf( TWO_PI / gyro->DefaultPeriod_x );
...
void Init_PeriodDetector( TPeriodDetector   *pd, Sint16 val1, float DefaultPeriod )
{
    pd->Period    = DefaultPeriod;
    pd->Frequency = ALGO_FREQUENCY / DefaultPeriod;

    pd->PeriodCountSumm = pd->Frequency / (float)val1;
    pd->PeriodCountSumm_invert = (float)1.0 / pd->PeriodCountSumm;
    pd->PeriodCounter   = 0;
    ...
    Init_ZForm( &pd->fi_period  , DefaultPeriod );
/*
    pd->fi_period.a[0] = (float)(0.028234550630192e-3 );
    pd->fi_period.a[1] = (float)(0.112938202520769e-3 );
    pd->fi_period.a[2] = (float)(0.169407303781153e-3 );
    pd->fi_period.a[3] = (float)(0.112938202520769e-3 );
    pd->fi_period.a[4] = (float)(0.028234550630192e-3 );
    pd->fi_period.b[0] = (float)( 1.000000000000000e+000);
    pd->fi_period.b[1] = (float)(-3.725359214892322);
    pd->fi_period.b[2] = (float)(5.243923243460126);
    pd->fi_period.b[3] = (float)(-3.304102245857121);
    pd->fi_period.b[4] = (float)(0.786011260553417);
*/
...
void Init_UART_Param( TGyro *gyro )
{
    ...
    gyro->UART_Param[ 2] = &gyro->ANodePeriod.Frequency;         // Фаза возбуждения Анала Х в Тактах алгоритма 

    gyro->UART_Param[ 3] = &gyro->pi_am_2.BlockOut;      // --- Коефициент разночастности ---
    gyro->UART_Param[ 4] = &gyro->pi_si_2.BlockOut;      // --- Квадратура ----
    gyro->UART_Param[ 5] = &gyro->NULL_value;//gyro->GyroOutRaw;   // Сырой выход гироскопа
    ...
    gyro->UART_Param[ 9] = &gyro->Phase;//XPhase.GlobalMean;//gyro->PhaseMean;//gyro->Zn_cos1.zn_curr;           // Фаза Х 

    gyro->UART_Param[10] = &gyro->ANodePeriod.pll_phase_incriment;//gyro->pi_phase_x.BlockOut;//gyro->YAmplitude;              //  Среднее значение Х
    gyro->UART_Param[11] = &gyro->fi_demod_c.BlockOut;//gyro->NULL_value;//gyro->XInputMean.GlobalMean;   //  Среднее значение Х
    gyro->UART_Param[12] = &gyro->fi_demod_q.BlockOut;//gyro->YInputMean.GlobalMean;   //  Среднее значение Y

    gyro->UART_Param[13] = &gyro->ANodePeriod.Period;// --- Средний период колебаний ---
    gyro->UART_Param[14] = &gyro->Zn_exc_x.zn_curr;           // Косинус в Тактах алгоритма 
    gyro->UART_Param[15] = &gyro->Zn_exc_y.zn_curr;//gyro->Zn_exc_y.zn_curr;//gyro->Zn_sin2.zn_curr;           // Косинус в Тактах алгоритма 
    gyro->UART_Param[16] = &gyro->Gx;//NULL_value;//gyro->pi_am_1.m_const;//gyro->Zn_cos1.zn_curr;           // Косинус в Тактах алгоритма 
    gyro->UART_Param[17] = &gyro->Gy;           // Косинус в Тактах алгоритма 
    gyro->UART_Param[18] = &gyro->NULL_value;           // Косинус в Тактах алгоритма 
