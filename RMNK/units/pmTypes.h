...
#define  MOD_2PI(a) ((a) >= TWO_PI) ? (a) = ((a)-TWO_PI) : (a)
...
typedef struct tag_RMNK
{
    ...
    float Frequency, PhaseIncriment, NCycles;

    float pi_pll_1[9], pi_pll_2[9];

    float fi_a[6], fi_b[6];
    
    float Qx[3], MjuX[3];

    float phi, delta_phi, base_sin, base_cos;
...
typedef struct tag_Gyro
{
    // ------ Temporary Variables -----
    float           NULL_value;
    float           c1, c2, c3, c5, c6, c7, c10, c11;
    TPhaseDetector  Phase;
    ...
    float   Base_ExcitationPhase_x;
    float   Base_ExcitationPhase_y;
    float        ExcitationPhaseAG;
    float        ExcitationPhase_x;
    float        ExcitationPhase_y;
	...
	float      fi_Phase[6];
    // ------ Regulators
    float pi_x_1[9], pi_x_2[9];
    float pi_y_1[9], pi_y_2[9];
    float pi_phi_1[9], pi_phi_2[9];
    ...
    float BaseAntinodePhase, kPhase;
