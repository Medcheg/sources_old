     ...
     CCollection(int aCollectionIndex = 0)
      {
         ...
         // ------
            gr_ValuesCount =  0;
            gr_DrawValues  = (TPoint*) calloc ( 4096, sizeof(TPoint) );
            gr_Values      = (float *) calloc ( 4096, sizeof(float ) );
            gr_ValuesIndex = (int   *) calloc ( 4096, sizeof(int   ) );
...
//            }
  ...
  //***********************************************************************************
    ...
    void __fastcall CalculateGraphicsValues( bool isDecimation, int DrawFromPoint, int DrawToPoint, int aPointsCount )
    {
       ...
       // --------
        ...
        bool   isMaximum_minus_Minimum_NoZero = (Maximum - Minimum == 0 && Maximum != 0);
        bool   isMaximum_minus_Minimum_Zero   = (Maximum - Minimum == 0 && Maximum == 0);
        double YScaleFactor   = ( isMaximum_minus_Minimum_NoZero == true || isMaximum_minus_Minimum_Zero == true) ? 0.001 : gr_Height / ( Maximum - Minimum );

       // --------
        gr_ValuesCount = 0;
        gr_CoefX       = 1.0f / (float)ArrayIndexStep;
        gr_ZeroPos = gr_Top + Maximum * YScaleFactor;
        ...
        for ( int xPos = 0; xPos < aPointsCount; xPos++)
        {
            ...
            else gr_DrawValues[xPos].y = gr_Top + (Maximum - gr_Values[xPos]) * YScaleFactor;
  ...
  //***********************************************************************************
    double Get_Mean( int FromIndex, int ToIndex )
    {
        ...
        if ( ToIndex   < 0 || ToIndex   > fValuesCount ) return -1;
          ...
          return ( (ToIndex - FromIndex) == 0) ? 0 : Result / (float)(ToIndex - FromIndex);
    }
  //***********************************************************************************
  ...
  //***********************************************************************************
    double Get_SCO( int FromIndex, int ToIndex, double Mean)
    {
       ...
       // ------
          for ( int i = FromIndex; i < ToIndex; i++ )
          {
                         float a = ( fValues[i] - Mean );
               SKO_Buffer += a*a;
          }

       // ------
          return ( (ToIndex - FromIndex) <= 1 ) ? 0 : sqrt( SKO_Buffer / (float)((ToIndex - FromIndex)-1)) ;
    }
  //***********************************************************************************
...
//***********************************************************************************
     ...
     __property TemplateType Values        [int index]  = { read = Get_Value          };
               ...
               // ---- Проверяю на контрольные слова -----
                    ...
                    else
                    {
                        if ( CheckControlWord( CharBuff1, "FileVersion", false) == true )
                        {
                            ...
                            if ( LocalResult != -1 && CheckControlWord( CharBuff4, "CanalCount"           , true) == false ) LocalResult = -1;
                            if ( LocalResult != -1 && CheckControlWord( CharBuff5, "CanalNames"           , true) == false ) LocalResult = -1;
                            if ( LocalResult != -1 ) if ( file_Param.FileVersion == 1 ) LocalResult = Load_ver_1(aFile, CanalCount, pbar);
                        }
                    }
          } catch ( ... ) {
               DeleteAllItems();
